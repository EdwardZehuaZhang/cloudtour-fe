...
export default function InviteAcceptPage() {
  ...
  useEffect(() => {
    async function fetchInvite() {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? ""}/api/invite/${token}`);
        if (!res.ok) {
  ...
  async function handleAccept() {
    setAccepting(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? ""}/api/invite/${token}`, { method: "POST" });

...
export function MembersPanel({ orgId, currentUserRole }: MembersPanelProps) {
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? ""}/api/orgs/${orgId}/members`);
      if (res.ok) {
  ...
  async function handleRemove(memberId: string) {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? ""}/api/orgs/${orgId}/members/${memberId}`, {
        method: "DELETE",
      });
